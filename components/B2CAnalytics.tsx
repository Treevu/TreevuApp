...
const B2CAnalytics: React.FC<Props> = ({ expenses, budget, subscriptionTier, onUpgrade }) => {
  ...
  const metrics = useMemo(() => {
    ...
    // Budget deviation logic with safety check for 0 budget
  ...
  return (
    <div className="space-y-6 animate-fadeIn">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <FlipCard themeColor="emerald" heightClass="h-48" frontContent={<div className="h-full flex flex-col justify-between relative overflow-hidden group"><div className="absolute top-0 right-0 opacity-10 group-hover:opacity-30 transition-opacity"><BoltIcon className="w-16 h-16 text-yellow-500 dark:text-accent" /></div><div><div className="flex items-center gap-1 mb-1"><h3 className="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Velocidad de Captura</h3></div><div className="flex items-baseline gap-1"><span className="text-4xl font-bold text-gray-900 dark:text-white">1.2s</span><span className="text-xs text-emerald-600 dark:text-emerald-400 font-bold">Récord</span></div></div><div><p className="text-[10px] text-gray-500 mt-2 mb-2">Tiempo de procesamiento OCR.</p><div className="w-full bg-gray-200 dark:bg-gray-800 h-1.5 rounded-full overflow-hidden"><div className="w-[90%] h-full bg-yellow-400 dark:bg-accent animate-pulse shadow-[0_0_10px_rgba(250,204,21,0.5)]"></div></div></div></div>} backContent="Nuestro motor de IA 'Flash' lee tus recibos en tiempo real." />

        {/* 2. RITMO DE GASTO */}
        <FlipCard themeColor="emerald" heightClass="h-48 md:col-span-1 lg:col-span-2" frontContent={<div className="h-full flex flex-col"><div className="flex justify-between items-start mb-3"><div className="flex items-center gap-1"><h3 className="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Frecuencia Semanal</h3></div><CalendarDaysIcon className="w-5 h-5 text-gray-400" /></div><div className="flex-1 flex items-end justify-between gap-2 pb-2">{['L','M','M','J','V','S','D'].map((day, i) => (<div key={i} className="flex-1 flex flex-col items-center gap-1 group/bar cursor-pointer h-full justify-end"><div className="w-full bg-gray-200 dark:bg-gray-800 rounded-t relative overflow-hidden transition-all hover:bg-gray-300 dark:hover:bg-gray-700 flex items-end" style={{ height: '100%' }}><div className="w-full bg-emerald-400/60 group-hover/bar:bg-emerald-500 transition-all" style={{ height: `${(metrics.dailyFreq[i] / metrics.maxDaily) * 100}%` }}></div></div><span className="text-[9px] text-gray-500 font-bold">{day}</span></div>))}</div></div>} backContent="Este histograma muestra qué días gastas más." />

        {/* 3. GASTO POR CATEGORÍA */}
        <FlipCard themeColor="emerald" heightClass="h-80 md:col-span-1 lg:col-span-1" frontContent={<div className="h-full flex flex-col"><div className="flex justify-between items-start mb-2"><div className="flex flex-col"><h3 className="text-sm font-bold text-gray-500 dark:text-gray-300 uppercase tracking-wider flex items-center gap-2">Gasto Total</h3><span className="text-3xl font-bold text-gray-900 dark:text-white">S/ {metrics.total.toFixed(0)}</span></div><div className="w-24"><Sparkline data={metrics.trendHistory} color="text-emerald-500" /></div></div><div className="h-px w-full bg-gray-200 dark:bg-white/10 mb-4"></div><div className="space-y-4 flex-1 overflow-y-auto pr-2 custom-scrollbar">{metrics.sortedCategories.length > 0 ? metrics.sortedCategories.map((item, idx) => (<div key={idx} className="group"><div className="flex justify-between text-xs mb-1"><span className="text-gray-700 dark:text-white font-bold tracking-wide truncate w-2/3">{item.cat}</span><span className="text-emerald-600 dark:text-emerald-300 font-mono">S/ {item.amount.toFixed(0)}</span></div><div className="w-full bg-gray-200 dark:bg-gray-800 rounded-full h-2 overflow-hidden border border-gray-300 dark:border-white/5"><div className="bg-emerald-500 dark:bg-emerald-400 h-full rounded-full shadow-[0_0_10px_rgba(52,211,153,0.5)] group-hover:bg-emerald-400 transition-colors" style={{ width: `${item.pct}%` }} /></div></div>)) : <div className="flex h-full items-center justify-center text-gray-500 text-sm italic">Registra gastos para ver tu desglose.</div>}</div></div>} backContent="Visualiza en qué categorías se concentra tu dinero." />

        {/* 4. EFICIENCIA FISCAL */}
        <FlipCard themeColor="emerald" heightClass="h-80 md:col-span-1 lg:col-span-1" frontContent={<div className="h-full flex flex-col"><div className="flex justify-between items-start mb-2"><div className="flex items-center gap-1"><h3 className="text-sm font-bold text-gray-500 dark:text-gray-300 uppercase tracking-wider">Eficiencia Fiscal</h3></div>{metrics.lostSavings > 0 && (<span className="bg-red-100 dark:bg-red-500/10 text-red-600 dark:text-red-400 text-[10px] font-bold px-2 py-1 rounded border border-red-200 dark:border-red-500/30 flex items-center gap-1 animate-pulse"><ExclamationTriangleIcon className="w-3 h-3" /> - S/ {metrics.lostSavings.toFixed(0)}</span>)}</div><div className="w-full h-8 mb-4"><Sparkline data={metrics.formalHistory} color="text-emerald-500" /></div><div className="flex-1 flex items-center justify-center relative my-2"><div className="relative w-32 h-32 rounded-full border-[10px] border-gray-200 dark:border-gray-800 flex items-center justify-center shadow-inner"><div className="absolute inset-0 rounded-full border-[10px] border-emerald-500 dark:border-emerald-400 border-l-transparent border-b-transparent transform -rotate-45 shadow-[0_0_15px_rgba(52,211,153,0.2)]" style={{ opacity: metrics.total > 0 ? (metrics.formalTotal / metrics.total) : 0.5 }}></div><div className="text-center z-10"><span className="block text-2xl font-bold text-gray-900 dark:text-white drop-shadow-sm">{metrics.total > 0 ? ((metrics.formalTotal / metrics.total) * 100).toFixed(0) : 0}%</span><span className="text-[8px] text-emerald-600 dark:text-emerald-400 uppercase font-bold tracking-widest">Formal</span></div></div></div></div>} backContent="Formal = Boleta Electrónica con DNI. Es la única forma de reducir impuestos y ganar más puntos." />

        {/* 5. RADAR DE AHORRO TRIBUTARIO */}
        <div className="md:col-span-1 lg:col-span-1">
            <FlipCard themeColor="emerald" heightClass="h-80" frontContent={<div className="h-full flex flex-col relative overflow-hidden"><div className="flex justify-between mb-4 relative z-10"><h3 className="text-sm font-bold text-emerald-600 dark:text-emerald-400 uppercase tracking-wider flex items-center gap-2"><DocumentMagnifyingGlassIcon className="w-5 h-5" /> Radar 3 UIT</h3>{!isPremium && <span className="text-[10px] text-black font-bold bg-yellow-400 dark:bg-accent px-2 py-1 rounded shadow-lg animate-pulse">Explorer</span>}</div><div className={`flex-1 flex flex-col justify-center items-center relative z-10 ${!isPremium ? 'blur-[2px]' : ''}`}><div className="text-center mb-4"><p className="text-gray-500 dark:text-gray-400 text-xs uppercase font-bold">Deducible Acumulado</p><p className="text-3xl font-bold text-gray-900 dark:text-white mt-1">S/ {metrics.currentDeductible.toFixed(2)}</p></div><div className="w-full bg-gray-200 dark:bg-gray-800 h-4 rounded-full overflow-hidden border border-gray-300 dark:border-white/5 mb-2 relative"><div className="absolute inset-0 bg-[url('https://www.transparenttextures.com/patterns/diagonal-stripes.png')] opacity-10"></div><div className="h-full bg-gradient-to-r from-emerald-600 to-emerald-400 shadow-[0_0_15px_rgba(52,211,153,0.5)] transition-all duration-1000" style={{ width: `${metrics.deductibleProgress}%` }}></div></div><div className="flex justify-between w-full text-[10px] text-gray-500 font-mono"><span>0%</span><span>Meta: S/ 15,450</span></div><p className="text-xs text-emerald-700 dark:text-emerald-300 mt-6 text-center px-2">{metrics.deductibleProgress < 20 ? "¡Pide boleta en restaurantes y hoteles!" : "¡Vas excelente con tus deducciones!"}</p></div>{!isPremium && (<div className="absolute inset-0 z-20 flex flex-col items-center justify-center bg-white/80 dark:bg-black/40 backdrop-blur-[1px]"><p className="text-gray-900 dark:text-white font-bold text-center px-6 text-sm mb-3">Calcula tu devolución de impuestos anual</p><button onClick={(e) => {e.stopPropagation(); onUpgrade();}} className="px-4 py-2 bg-yellow-400 dark:bg-accent text-black font-bold rounded-full text-xs hover:scale-105 transition-transform shadow-lg">Ver Mi Radar</button></div>)}</div>} backContent="En Perú, puedes deducir hasta 3 UIT (aprox. S/ 15k) de tus impuestos anuales si pides boleta en rubros seleccionados." />
        </div>

        {/* 6. DESVIACIÓN VS IA (ENRICHED) */}
        <div className="md:col-span-1 lg:col-span-1 md:col-start-2 lg:col-start-auto">
             {isPremium ? (

      ...
      case UserRole.EMPLOYER:
        return {
          ...
          backData: [
            {
              l: "AHORRO RETENCIÓN",
              v: `S/ ${(companyKPIs.retentionSavings / 1000).toFixed(1)}k`,
            },
            {
              l: "RIESGO FUGA",
              v: `${companyKPIs.flightRiskScore}%`,
              color: "text-red-400",
            },
          ...
          backData: [
            {
              l: "INGRESOS",
              v: `S/ ${offers.reduce(
                (a, b) => a + (b.revenueGenerated || 0),
                0
              )}`,
            },
            {
              l: "REDENCIONES",
              v: offers.reduce((a, b) => a + b.redemptions, 0),
            },
            {
              l: "TRÁFICO HOY",
              v: hourlyTraffic.reduce((a, b) => a + b.volume, 0),
            },
          ],
        };
    ...
    <div className="w-full max-w-md mx-auto perspective-1000 my-6 group z-20 relative">
      ...
      <div className="animate-float">
        ...
        >
          {/* --- FRONT FACE --- */}
          <div
            className={`absolute inset-0 w-full h-full rounded-2xl bg-gradient-to-br ${config.gradient} border ${config.border} shadow-2xl overflow-hidden backface-hidden`}
          >
            {/* Holographic Sheen Overlay */}
            <div
              className="absolute inset-0 opacity-40 pointer-events-none mix-blend-overlay"
              style={{
                background: `linear-gradient(115deg, transparent 0%, rgba(255,255,255,0.4) ${
                  40 + rotation.y
                }%, transparent 60%)`,
              }}
            />
            ...
            {(config as any).isFounder && (
              <div className="absolute top-4 left-1/2 -translate-x-1/2 bg-gradient-to-r from-amber-200 via-yellow-400 to-amber-200 text-black text-[9px] font-extrabold px-3 py-1 rounded-full tracking-[0.2em] shadow-[0_0_20px_rgba(250,204,21,0.6)] z-20 border border-white/50 whitespace-nowrap">
                MIEMBRO FUNDADOR
              </div>
            )}
            ...
            <div className="relative z-10 p-6 flex flex-col justify-between h-full">
              <div className="flex justify-between items-start mt-4">
                ...
                <div className="flex items-center gap-3">
                  <div
                    className={`w-10 h-8 rounded-md ${config.chipColor} shadow-inner border border-white/20 relative overflow-hidden`}
                  >
                ...
                <div className="text-right">
                  <h3 className="text-white font-bold italic tracking-widest text-lg">
                    treevü
                  </h3>
                  <p
                    className={`text-[10px] font-bold tracking-[0.2em] ${config.accent}`}
                  >
                    {config.title}
                  </p>
                </div>
              ...
              <div className="flex justify-between items-end">
                <div>
                  <p className="text-[8px] text-gray-400 font-bold uppercase tracking-widest mb-0.5">
                    {config.frontData.label1}
                  </p>
                  <p className="text-sm text-white font-medium tracking-wide shadow-black drop-shadow-md uppercase">
                    {config.frontData.val1}
                  </p>
                </div>
                <div className="text-right">
                  <p className="text-[8px] text-gray-400 font-bold uppercase tracking-widest mb-0.5">
                    {config.frontData.label2}
                  </p>
                  <p
                    className={`text-sm font-bold tracking-wide shadow-black drop-shadow-md uppercase ${config.accent}`}
                  >
                    {config.frontData.val2}
                  </p>
                </div>
              </div>
            </div>
          ...
          {/* --- BACK FACE --- */}
          <div
            className={`absolute inset-0 w-full h-full rounded-2xl bg-gray-900 border ${config.border} shadow-2xl overflow-hidden backface-hidden rotate-y-180`}
          >
            {/* Magnetic Strip */}
                ...
                {config.backData.map((item: any, idx) => (
                  <div
                    ...
                    className={`flex justify-between items-end border-b border-white/5 pb-1 ${
                      item.action
                        ? "cursor-pointer hover:bg-white/5 rounded px-1 -mx-1 transition-colors"
                        : ""
                    }`}
                    onClick={(e) => {
                      if (item.action) {
              ...
              <div className="ml-4 flex flex-col items-center justify-center gap-3 border-l border-white/10 pl-4">
                ...
                {role === UserRole.EMPLOYEE && (
                  <div className="flex flex-col items-center text-center opacity-60">
                    ...
                    <p className="text-[7px] text-emerald-400 font-bold uppercase leading-tight">
                      BÚNKER
                      <br />
                      DE DATOS
                    </p>
                    <p className="text-[6px] text-gray-500 uppercase mt-1">
                      OFFLINE MODE
                    </p>
                  </div>
                )}
